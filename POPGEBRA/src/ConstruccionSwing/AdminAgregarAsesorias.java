...
public class AdminAgregarAsesorias extends JPanel {
    private final Runnable onBack;
    private final TamañosColoresPosicion valores = new TamañosColoresPosicion();

    private JComboBox<ComboItem> cmbAsesores;
    ...
    public AdminAgregarAsesorias(Runnable onBack) {
        this.onBack = onBack;
        ...
        JLabel titulo = new JLabel("Crear Nueva Asesoría (Admin)");
        ...
        panel.add(buildSeleccionAsesor());
    ...
    private JPanel buildSeleccionAsesor() {
        ...
        gbc.gridx = 0;
        gbc.gridy = 0;
        JLabel lblAsesor = new JLabel("Seleccionar Asesor:");
        lblAsesor.setFont(labelFont);
        panel.add(lblAsesor, gbc);

        gbc.gridx = 1;
        cmbAsesores = new JComboBox<>();
        cmbAsesores.setFont(labelFont);
        cmbAsesores.setPreferredSize(new Dimension(300, 35));
        cargarAsesores();
        cmbAsesores.addActionListener(e -> cargarMateriasAsesor());
        panel.add(cmbAsesores, gbc);
    ...
    private JPanel buildHorarios() {
        ...
        scrollHorarios.setPreferredSize(new Dimension(850, 120));
    ...
    private void cargarAsesores() {
        ...
        List<ComboItem> asesores = dao.getAllAsesores();
        for (ComboItem asesor : asesores) {
            cmbAsesores.addItem(asesor);
        }
    }

    private void cargarMateriasAsesor() {
        cmbMaterias.removeAllItems();
        ComboItem asesorSeleccionado = (ComboItem) cmbAsesores.getSelectedItem();
        if (asesorSeleccionado == null) return;

        String matriculaAsesor = asesorSeleccionado.getKey();
    ...
    private void guardarAsesoria() {
        if (cmbAsesores.getSelectedItem() == null || cmbMaterias.getSelectedItem() == null) {
            JOptionPane.showMessageDialog(this,
                    "Por favor selecciona un asesor y una materia.",
        ...
        if (nombre.isEmpty() || periodo.isEmpty()) {
            JOptionPane.showMessageDialog(this,
